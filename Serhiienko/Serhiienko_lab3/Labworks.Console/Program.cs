...
namespace Labworks.ConsoleApp
{
    ...
    class Program
    {
        ...
        static void LaunchLabwork3(int cities, int populationSize)
        {
            var fileContent = new ConcurrentQueue<string>();
            ...
            var mutation = new TworsMutation();
            var fitness = new TspFitness(cities, 0, 1000, 0, 1000);
            var population = new GeneticSharp.Domain.Populations.Population(populationSize, populationSize * 2, new TspChromosome(cities));
            ...
            {
                foreach (TspChromosome solution in ga.Population.CurrentGeneration.Chromosomes)
                {
                    fileContent.Enqueue(string.Format("c\t{0}\t{1}\t{2}\t{3}\t{4}"
                        , "-"
                        , solution.Distance
                    ...
                    foreach (var gene in solution.GetGenes())
                    {
                        fileContent.Enqueue(string.Format("c\t{0}\t{1}\t{2}\t{3}\t{4}"
                            , "-"
                            , fitness.Cities[(int)gene.Value].X
                            , fitness.Cities[(int)gene.Value].Y
                            , "-"
                            , "-"
                            ));
                    }
                ...
                TspChromosome bestSolution = (TspChromosome)ga.Population.CurrentGeneration.BestChromosome;
                ...
                foreach (var gene in bestSolution.GetGenes())
                {
                    fileContent.Enqueue(string.Format("c\t{0}\t{1}\t{2}\t{3}\t{4}"
                        , "->"
                        , fitness.Cities[(int)gene.Value].X
                ...
                Console.CursorLeft = 0;
                Console.WriteLine("Generations: {0}", ga.Population.GenerationsNumber);
                Console.WriteLine("Fitness: {0}", bestChromosome.Fitness);
                Console.WriteLine("Length: {0}", tspSolution.Length);
                Console.WriteLine("Time: {0}", ga.TimeEvolving);
            };
            ...
            var di = Directory.CreateDirectory("lab-3");
            Directory.SetCurrentDirectory(di.FullName);
                ...
                switch (entry[0])
                {
                    case 'c':
                        if (openNewFile)
                        {
                            if (fileStreamGnrs != null)
                                fileStreamGnrs.Close();
                            fileStreamGnrs = new StreamWriter("lab-3-generation-" + gi++ + ".txt");
                            openNewFile = false;
                        }
        ...
        static void Main(string[] args)
        {
            ...
            //LaunchLabwork3(16, 16);
            //LaunchLabwork3(16, 32);
            LaunchLabwork3(16, 64);
            //LaunchLabwork3(32, 16);
            //LaunchLabwork3(32, 32);
            //LaunchLabwork3(32, 64);
            //LaunchLabwork3(64, 16);
            //LaunchLabwork3(64, 32);
            //LaunchLabwork3(64, 64);
        }
    }
}
